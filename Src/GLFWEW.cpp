...
bool Window::GetCommand(COMMANDLIST command)
{
	...
	if (command==COMMANDLIST_DECISION) {
		//決定キーの処理,
		if (IsKeyDown(GLFW_KEY_ENTER) || gamepad.buttonDown & GamePad::A) {
	...
	* 
	*/
	bool Window::CheckPos()
	{
		if (GetCommand(COMMANDLIST_DECISION)) {
			int posX = 0, posY = 0;
			glfwGetWindowPos(window,&posX,&posY);
			std::cout << "posX=" << posX << " and posY=" << posY << std::endl;

			int sizeX = 0, sizeY = 0;
			glfwGetWindowSize(window,&sizeX,&sizeY);
			std::cout << "sizeX=" << sizeX << " and sizeY=" << sizeY << std::endl;
			
			int frameX = 0, frameY = 0;
			glfwGetFramebufferSize(window,&frameX,&frameY);
			std::cout << "frameX=" << frameX << " and frameY=" << frameY << std::endl;
			
			//ウィンドウのレンダーバッファのサイズを変更.
